...
export default class Television extends THREE.Group {
  ...
  addParts() {
    ...
    const ventilationMaterial = new THREE.MeshPhongMaterial({
      ...
      bumpMap: new THREE.TextureLoader().load(
        "src/images/corpusVentilation.png"
      ),
      bumpScale: 1.0,
    });
    ...
    const hollowCorpus = CSG.toMesh(
      corpusCSG.subtract(cavityCSG),
      corpus.matrix,
      corpus.material
    );
    ...
    const ventilation = new THREE.Mesh(
      ventilationGeometry,
      ventilationMaterial
    );
    ...
    const knobGripGeometry = new THREE.BoxGeometry(0.4, 1, 3).translate(
      0,
      1,
      0
    );
    const powerKnob = new THREE.Mesh(knobGeometry, metalMaterial).add(
      new THREE.Mesh(knobGripGeometry, metalMaterial)
    );
    ...
    // --------------------
    const powerKnobAnimation = new Animation(
      powerKnob,
    ...
    // --------------------
    const volumeKnobAnimation = new Animation(
      volumeKnob,
      AnimationType.ROTATION,
      AnimationAxis.Y
    );
    ...
    // Antenna
    // -------
    const antennaProfile = new THREE.Shape().absellipse(
      0,
      0,
      0.2,
      0.2,
    ...
    const antennaGeometry = new THREE.ExtrudeGeometry(
      antennaProfile,
      extrudeSettings
    );
    ...
    // Antenna Animation
    // -----------------
    antenna.tweenAnimationUp = new TWEEN.Tween(antenna.rotation)
      .to(
        new THREE.Vector3(
    ...
    antenna.tweenAnimationDown = new TWEEN.Tween(antenna.rotation)
      .to(
        new THREE.Vector3(
          antenna.rotation.x,
          antenna.rotation.y,
  ...
  updateFunctionalState() {
    const powerOn =
      THREE.MathUtils.radToDeg(this.children[3].children[1].rotation.y) === -90;
    const volumeHigh =
      THREE.MathUtils.radToDeg(this.children[3].children[2].rotation.y) === -90;
    const antennaUp =
      THREE.MathUtils.radToDeg(this.children[5].rotation.x) === 0;
